...
class Proxy {
public:
    Proxy(int8_t* ptr, size_t ind) : ptr_(ptr), ind_(ind) {};    
...
Proxy::operator bool() const {
    return ((*ptr_ << (7 - ind_)) >> 7);
}


Proxy& Proxy::operator=(bool other) {
    *ptr_ ^= (*ptr_) & (~(1 << (7 - ind_)) | (other << (7 - ind_)));
    return *this; 
}
...
const bool my_array<bool, N>::at(size_t index) const{
    assert(index >= 0 && index < N);    
    return ((array_[index / 8] << (7 - index % 8)) >> 7);
}


template <size_t N>
Proxy my_array<bool, N>::at(size_t index) {
    assert(index >= 0 && index < N);    
...
template <size_t N>
Proxy my_array<bool, N>::operator[] (size_t index) {
    return Proxy(array_ + index/8, index % 8);
}
